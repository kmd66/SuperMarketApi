...
IF EXISTS(SELECT 1 FROM sys.procedures WHERE [object_id] = OBJECT_ID('prd.spModifyClassification'))
	DROP PROCEDURE prd.spModifyClassification
GO

CREATE PROCEDURE prd.spModifyClassification
	@AIsNewRecord BIT,
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			SET @ParentNode = COALESCE((SELECT TOP 1 [Node] FROM prd.Classification WHERE [GuID] = @ParentID AND Removed = 0), 0x)
			SET @ParentNodeLevel = COALESCE((SELECT TOP 1 [Node].GetLevel() + 1 FROM prd.Classification WHERE [GuID] = @ParentID AND Removed = 0), 1)
		END
		...
		BEGIN
			...
			SET @LastChildNode = (
				SELECT MAX([Node]) FROM prd.Classification 
			...
			INSERT INTO prd.Classification
