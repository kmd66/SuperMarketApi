...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.fnGregorianToJalali') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION dbo.fnGregorianToJalali
GO

CREATE FUNCTION dbo.fnGregorianToJalali (@Date smalldatetime)
RETURNS nvarchar(20)
AS
BEGIN
	...
	IF dbo.fnIsJalaliLeapYear(@Year - 1) = 1 AND dbo.fnIsLeapYear(YEAR(@Date)) = 1
		SET @ToEid = 80
	ELSE
		SET @ToEid = 79
	...
	BEGIN
		...
		IF dbo.fnIsJalaliLeapYear(@Year) = 1 AND dbo.fnIsLeapYear(YEAR(@Date)) = 0
	...
	IF dbo.fnIsJalaliLeapYear(@Year) = 1
	BEGIN
